...
export default function SettingsPage() {
  ...
  const save = async () => {
    ...
    const payload = { ...form }
    const apiKeys = [
      'openrouter_api_key', 'gemini_api_key', 'grok_api_key', 'pexels_api_key',
      'pixabay_api_key', 'jamendo_client_id', 'acoustid_api_key', 'huggingface_token',
      'amazon_pa_api_key', 'amazon_pa_secret', 'amazon_affiliate_tag', 'reddit_client_id',
      'reddit_secret', 'news_api_key'
    ] as const
    apiKeys.forEach(k => { if (!payload[k]) delete payload[k] })
    try {
        ...
        <div className="space-y-6">
          ...
          <div className="card">
            ...
            <div className="space-y-3 font-mono text-sm">
              {[
                { id: 'openrouter_api_key', label: 'OpenRouter API Key', mappedKey: 'openrouter' },
                { id: 'gemini_api_key', label: 'Gemini API Key', mappedKey: 'gemini' },
                { id: 'grok_api_key', label: 'Grok API Key', mappedKey: 'grok' },
                { id: 'pexels_api_key', label: 'Pexels API Key', mappedKey: 'pexels' },
                { id: 'pixabay_api_key', label: 'Pixabay API Key', mappedKey: 'pixabay' },
                { id: 'jamendo_client_id', label: 'Jamendo Client ID', mappedKey: 'jamendo' },
                { id: 'acoustid_api_key', label: 'AcoustID API Key', mappedKey: 'acoustid' },
                { id: 'huggingface_token', label: 'Hugging Face Token', mappedKey: 'huggingface' },
                { id: 'amazon_pa_api_key', label: 'Amazon PA API Key', mappedKey: 'amazon_pa' },
                { id: 'amazon_pa_secret', label: 'Amazon PA Secret', mappedKey: 'amazon_pa_secret' },
                { id: 'amazon_affiliate_tag', label: 'Amazon Affiliate Tag', mappedKey: 'amazon_tag' },
                { id: 'reddit_client_id', label: 'Reddit Client ID', mappedKey: 'reddit_client' },
                { id: 'reddit_secret', label: 'Reddit Secret', mappedKey: 'reddit_secret' },
                { id: 'news_api_key', label: 'News API Key', mappedKey: 'news' }
              ].map(({ id, label, mappedKey }) => (
                <div key={id} className="flex flex-col gap-1">
                  <label className="text-xs text-zinc-400">{label}</label>
                  <input
                    type="password"
                    className="input font-sans text-sm"
                    placeholder={s.keys_masked[mappedKey] ? `Set (${s.keys_masked[mappedKey]}) - Type to overwrite` : 'Not set'}
