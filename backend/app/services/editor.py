...
def _ass_path_for_filter(p: Path) -> str:
    """FFmpeg's `ass` filter is notoriously strict about Windows paths. We must
    escape `:` and `'` so the path is safe inside `ass='...'`.
    By making the path relative to the current working directory, we avoid
    escaping issues with apostrophes in the user's home directory (e.g. LAPTOP'S).
    """
    try:
...
async def render_video(
    ...
    concat_file.write_text(
        "".join(f"file '{concat_escape(p.as_posix())}'\n" for p in normalized), encoding="utf-8"
    )
    ...
    for i, sc in enumerate(scenes):
        ...
        entries.append(f"file '{concat_escape(vp.as_posix())}'")
        ...
        if i < len(scenes) - 1:
            entries.append(f"file '{concat_escape(gap.as_posix())}'")
